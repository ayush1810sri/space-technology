...
const InnovationShowcase = () => {
  ...
  // Fetch student projects from the backend API
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const projects = await api.fetchSpaceTechItems();
        // Filter projects by category to match the original showcase
        const innovationProjects = projects.filter(project =>
          project.category === 'Satellite Technology' ||
          project.category === 'Life Support Systems' ||
          project.category === 'Space Safety' ||
          project.category === 'Additive Manufacturing' ||
          project.category === 'Life Sciences' ||
          project.category === 'Propulsion'
        );
        setStudentProjects(innovationProjects);
      } catch (error) {
    ...
    <div className="max-w-7xl mx-auto px-4">
            ...
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${
                  ...
                  project.status === 'Retired' ? 'bg-red-900 text-red-300' :
              ...
              <p className="text-gray-300 mb-1">{project.team || "Team"}</p>
      ...
      {showDetails && selectedProject && (
        ...
        >
          ...
          >
            ...
            <h3 className="text-2xl font-bold mb-2">{selectedProject.name || selectedProject.title}</h3>
            <p className="text-blue-400 mb-1">{selectedProject.team || "Team"}</p>
            <p className="text-gray-200 mb-6">{selectedProject.description}</p>

            <div className="mb-6">
              <h4 className="text-lg font-bold mb-3">Project Details</h4>
              <p className="text-gray-200 mb-4">{selectedProject.access?.details || selectedProject.description}</p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  <h5 className="font-bold text-gray-300 mb-2">University</h5>
                  <p className="text-gray-200">{selectedProject.access?.university || "University not specified"}</p>
                </div>
                <div>
                  <h5 className="font-bold text-gray-300 mb-2">Category</h5>
                  <p className="text-gray-200">{selectedProject.category}</p>
                </div>
              </div>

              <div className="mb-4">
                <h5 className="font-bold text-gray-300 mb-2">Team Members</h5>
                <ul className="list-disc pl-5 text-gray-200">
                  {selectedProject.access?.teamMembers?.map((member, index) => (
                    <li key={index}>{member}</li>
                  )) || <li>Team members not specified</li>}
                </ul>
              </div>
            </div>

            <div className="flex flex-wrap gap-3">
              {selectedProject.access?.github ? (
                <a
      ...
      >
        ...
        <button
          ...
          onClick={() => console.log('Submit Project button clicked')}
        >
          Submit Project
        </button>
      </motion.div>
